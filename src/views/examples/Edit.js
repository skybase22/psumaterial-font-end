...
class Edit extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    await firebase.database().ref("listNumber").once('value', async (snapshot) => {
      if (snapshot.exists()) {
        ...
        await this.setState({ lastNumber: 0 })
      }
    })
    ...
    await firebase.database().ref("listResult").once('value', async (snapshot) => {
      if (snapshot.exists()) {
        ...
        await this.setState({ lastResult: 0 })
      }
    })
  ...
  getMaterial = async () => {
    if (this.state.selectMaterial !== "") {
      ...
      this.setState({
        urlQR: "https://psupktmaterial.firebaseapp.com/detail?id=" + query
      })
  ...
  onUpdate2 = async (event) => {
    if (this.state.durableCode !== "" && this.state.id !== "") {
      ...
      await firebase.database().ref("myURL/" + this.state.id).set({
        id: this.state.id,
                ...
                <Row>
                  ...
                  <Col lg="6" md="6">
                    {this.state.visible === false ? (
                      <Row className="" s style={{ marginBottom: "3rem", minHeight: "55px" }} >
                        <div>
